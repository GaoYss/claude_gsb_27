...
| 绿植更换 | `/replacements` | 更换登记（植株、类别、规格、数量、原因、原植株状况、供苗单位、单价与金额、登记人）、来源任务/养护记录双向反查、按类别/原因统计与占比、按绿地/日期区间筛选、**费用按绿地×月/季/年归属报表**、**批次号幂等的批量导入（重复明细自动判重）** |
...
│   │                            #   replacement 含 CostReportDialog（费用归属）与 ReplacementImportDialog（批量导入）
...
| GET/PUT/DELETE | `/maintenance-tasks/{id}` | 任务详情（含执行进度、养护记录与**关联更换明细**） / 更新 / 删除（有记录或更换时需 `force`，记录与更换保留但解除关联并留存快照） |
...
| GET/PUT/DELETE | `/maintenance-records/{id}` | 记录详情（含关联更换记录） / 更新 / 删除（更换记录保留并留存记录编号快照） |
| GET | `/maintenance-records/summary` | 记录汇总（条数、工时、质量分布） |
| GET/POST | `/plant-replacements` | 更换记录列表（`task_id`/`green_space_id`/`maintenance_record_id`/`plant_category`/`reason`/`supplier`/日期区间，返回汇总；列表带来源任务与删除快照标记） / 登记更换 |
| GET/PUT/DELETE | `/plant-replacements/{id}` | 详情 / 更新 / 删除 |
| GET | `/plant-replacements/summary` | 更换汇总（按植物类别、更换原因） |
| GET | `/plant-replacements/cost-report` | 更换费用归属：按绿地 × `group_by=month|quarter|year` 汇总金额/数量/条数，筛选条件与列表一致，金额可下钻到同一批明细 |
| POST | `/plant-replacements/import` | 批量导入更换明细：`batch_no` 幂等键 + `items[]`；同批次重复提交整批重放不二次入账，批次内/跨批次内容重复自动判重，逐行返回行号错误 |
...
3. **绿地归属一致性**：养护记录可只填绿地（日常养护）或只填任务（绿地自动跟随任务）；两者同时提供时必须属于同一绿地。更换记录若关联养护记录，必须是同一绿地的记录；关联记录后其**任务外键与任务编号/名称快照自动跟随**，无需手工指定。
4. **日期约束**：养护日期、更换日期不得早于绿地建成日期。
5. **金额核算与费用归属**：更换金额 = 数量 × 单价，由后端统一计算；未填单价时金额留空，前端提示补录。费用一律以 `green_space_id` + `replace_date` 归属，`/plant-replacements/cost-report` 按绿地 × 月/季/年汇总，筛选条件与列表完全一致，报表金额可下钻到同一批明细。
6. **删除保护与关联溯源**：
   - 删除绿地时若已存在任务/记录/更换数据会返回 409 并给出数量明细，需 `force=true` 才级联删除；
   - 删除任务时若已关联养护记录或更换明细会返回 409（明细含两类数据数量），需 `force=true`；强制删除后**养护记录与更换记录全部保留**，仅解除任务关联；更换明细会冻结任务编号/名称快照（`task_snapshot`），事后仍能看出出自哪个任务，不因任务删除而含糊；
   - 删除养护记录时其名下更换记录保留、解除记录外键，并留存记录编号快照（`record_snapshot`）；
   - 养护记录改挂/移出任务时，其名下更换明细的任务关联与快照同步重算（任务仍存在时不显示"已删除"）。
7. **批量导入幂等**：更换记录支持 `POST /plant-replacements/import` 批量导入。以 `batch_no` 为幂等键，同一批次号重复提交整批重放首次结果、不二次入账；批次内与跨批次的重复内容按内容指纹（绿地、植株、规格、数量、单价、更换日期、供苗单位等）自动判重跳过；任一行校验失败不影响其它行，错误按行号返回。
...
| `plant_replacement` | 绿植更换记录 | `replacement_no`(唯一)、`green_space_id`、`maintenance_record_id`(可空)、`task_id`(可空)、`record_snapshot_no`、`task_snapshot_no/title`、`plant_name`、`plant_category`、`quantity`、`unit`、`reason`、`unit_price`、`amount`、`supplier`、`operator`、`content_fingerprint`(唯一)、`import_batch_id` |
| `plant_replacement_import_batch` | 更换批量导入批次 | `batch_no`(唯一，幂等键)、`source`、`total_rows`、`imported_count`、`duplicate_count`、`failed_count`、`result_detail`(JSON) |

关联与溯源：更换记录的 `green_space_id` 必填（费用归属依据）；`task_id`、`maintenance_record_id` 为可空外键，来源被删除时由 service 层显式置空并分别冻结 `task_snapshot_*` / `record_snapshot_no` 快照，保证任务或养护记录删除后更换明细的出处仍可追溯；`content_fingerprint` 全局唯一，作为批量导入跨批次去重的依据。绿地删除时任务/记录/更换级联清理；任务与养护记录之间为可空外键（`SET NULL`）。

## 八、测试

```bash
cd backend
python -m pytest              # 66 个用例：接口、校验、跨模块规则、端到端流程、关联溯源与幂等导入
```

覆盖重点：绿地编号生成与唯一性、枚举与字段校验、列表过滤/排序/分页、任务状态自动流转与手动流转限制、记录删除后的状态回退、更换金额核算、**更换→绿地/记录/任务/登记人/供苗单位顺向关联与任务/记录反查更换明细、删任务/删记录后的快照溯源、费用按绿地×时间段归属、批量导入批次幂等与内容指纹判重**、删除保护与强制删除、统计聚合口径一致性、演示数据自洽性。
